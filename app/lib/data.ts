...
export async function fetchCardData() {
  try {
    ...
    const invoiceStatusPromise = client.sql<InvoiceStatus[]>`SELECT
    ...
    const totalPaidInvoices = formatCurrency(Number(invoiceStatusData[0]?.paid ?? 0));
    const totalPendingInvoices = formatCurrency(Number(invoiceStatusData[0]?.pending ?? 0));
